...
resume = st.text_area("🧾 Paste your Resume here", height=300, value= extracted_resume, help="Copy and paste your resume.")
...
job_desc = st.text_area("📌Paste the Job Description here", height=300, value= extracted_jd, help="Copy and paste the full job post.")
...
with col1:
    generate_cl = st.checkbox("📨Cover letter", value=True)
with col2:
...
if st.button("🚀 Boost My Application"):
    if not resume or not job_desc:
        ...
        with st.spinner("  Gemini is generating..."):
            ...
            if generate_it:
                items.append("### Interview Tips\nList 3-5 concise interveiw preparation tips specific to this Role.")

            full_prompt = f"""
            You are a professional career coach and an expert in job applications.
            Given the following resume and job description, please generate the requested items in clearly seperate sections using headings (###).
            Ensure the iutputs are professional, concise, and highly relevant
...
st.markdown("---")
st.caption("Built with ❤️ by RAJI")
